...
class DistanceUtil:
    ...
    def findEdgesWithMinDistance(self, coordinate: list, num: int) -> list:
        '''
            Find Top five nearest edges
        '''
        [latH, lonH] = coordinate
        ...
        # Compute all distance from the point to edge
        all_Distance = LX * lens + RX * lend + (1 - LX - RX) * Heights
        indexs = all_Distance.argsort()[:num]
    
        # return source node and destination node
        nearest_edges = [(int(self.edges[index, 0].cpu().item()), int(self.edges[index, 1].cpu().item())) for index in indexs]
